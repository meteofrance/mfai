...
def walk_folder_for_py2puml(root_dir_path: str) -> Iterator[tuple[str, str]]:
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir_path):
        ...
        for filename in filenames:
            ...
            if (path, module_path) in visited:
                continue
            visited.add((path, module_path))
...
def generate_diagram(path_and_name: tuple[str, str]) -> None:
    ...
    print(f"Generating diagram: {path}")
    ...
    save_folder = Path(path[5:]).parent
...
if __name__ == '__main__':
    ...
    with Pool(10) as pool:
        pool.map(generate_diagram, walk_folder_for_py2puml('mfai'))
