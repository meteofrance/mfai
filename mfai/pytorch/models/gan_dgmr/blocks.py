...
class ContextConditioningStack(torch.nn.Module):
    ...
    def forward(self, x: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        for i in range(steps):
            ...
            scale_3.append(s3)
            scale_4.append(s4)
        tensor_1 = torch.stack(
            scale_1, dim=1
        )  # B, T, C, H, W and want along C dimension
        tensor_2 = torch.stack(
            scale_2, dim=1
        )  # B, T, C, H, W and want along C dimension
        tensor_3 = torch.stack(
            scale_3, dim=1
        )  # B, T, C, H, W and want along C dimension
        tensor_4 = torch.stack(
            scale_4, dim=1
        )  # B, T, C, H, W and want along C dimension
        # Mixing layer
        tensor_1 = self._mixing_layer(tensor_1, self.conv1)
        tensor_2 = self._mixing_layer(tensor_2, self.conv2)
        tensor_3 = self._mixing_layer(tensor_3, self.conv3)
    ...
    def _mixing_layer(self, inputs: Tensor, conv_block: torch.nn.Module) -> Tensor:
        ...
        # Convert from [batch_size, time, c, h, w] -> [batch_size, c * time, h, w]
...
class LatentConditioningStack(torch.nn.Module):
    ...
    def __init__(
        ...
        self.l_block1 = LBlock(
            input_channels=self.input_channels, output_channels=output_channels // 32
        )
